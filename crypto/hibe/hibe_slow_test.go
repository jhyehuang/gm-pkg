...
import (
	...
	"github.com/jhyehuang/gm-pkg/crypto/hibe/hibe_noamd64/hibe"
...
func testEncrypt2HibeMsgMapWithUnsupportData(t *testing.T, plaintext []byte) {
	tests := []struct {
		...
		{ //
			...
			wantError:  fmt.Sprintf("ID [%s] is matched, but Params are different, please check it", topLId),
		},
		{ //
			plaintext:  plaintext,
...
func testDecryptHibeMsgMapWitUnsupportData(t *testing.T, hibeMap map[string]string, symKeyType crypto.KeyType) {
	...
	tests := []struct {
		...
		{
			localId:   errFmtIDs[0],
			wantError: "invalid parameters, id is nil",
		},
		{
			localId:   errFmtIDs[1],
