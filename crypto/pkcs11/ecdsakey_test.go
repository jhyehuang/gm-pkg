...
func TestNewPrivateKey_SM2(t *testing.T) {
	if !support_GM {
		t.Skipf("skip: softhsm not supported sm2")
	}
	privKeyId := string(internalSM2KeyLabel)
	bcPriv, err := NewPrivateKey(p11, privKeyId, bccrypto.SM2)
...
func TestP11Handle_SignVerify(t *testing.T) {
	...
	priv, err := p11.findPrivateKeyByLabel(internalSM2KeyLabel)
	assert.NoError(t, err)
	assert.NotNil(t, priv)
	...
	pub, err := p11.findPublicKeyByLabel(internalSM2KeyLabel)
	assert.NoError(t, err)
	assert.NotNil(t, pub)
...
func TestP11Handle_ExportECDSAPublicKey(t *testing.T) {
	...
	//hsm verify
	pubOjb, err := p11.findPublicKey(internalSM2KeyLabel)
	err = p11.Verify(*pubOjb, pkcs11.NewMechanism(CKM_SM3_SM2_DER, nil), plain, sig)
	assert.NoError(t, err)

	//soft verify
	pub, err := p11.ExportECDSAPublicKey(internalSM2KeyLabel, SM2)
	assert.NoError(t, err)
	assert.NotNil(t, pub)
...
func TestHSMVerify_SM2(t *testing.T) {
	...
	for i := 0; i < testNum; i++ {
		...
		pub, err := p11.findPublicKey(internalSM2KeyLabel)
		assert.NoError(t, err)
		assert.NotNil(t, pub)
		err = p11.Verify(*pub, pkcs11.NewMechanism(CKM_SM3_SM2_APPID1_DER, nil), plain, sig)
		assert.NoError(t, err)
