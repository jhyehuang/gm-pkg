...
func TestP11Handle_InternalAES(t *testing.T) {
	sk, err := NewAESKey(p11, internalAESKeyLabel)
...
func TestNewSecretKey_AES(t *testing.T) {
	sk, err := NewSecretKey(p11, string(internalAESKeyLabel), bccrypto.AES)
	assert.NoError(t, err)
	assert.NotNil(t, sk)
...
func TestGenerateSecretKey_AES_16(t *testing.T) {
	keyLabel := fmt.Sprintf("MasterKey_AES_16_%d", incNextId())
...
func TestGenerateSecretKey_AES_24(t *testing.T) {
	keyLabel := fmt.Sprintf("MasterKey_AES_24_%d", incNextId())
...
func TestGenerateSecretKey_AES_32(t *testing.T) {
	keyLabel := fmt.Sprintf("MasterKey_AES_32_%d", incNextId())
